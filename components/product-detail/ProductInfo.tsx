    ...
    return Array.from({ length: 5 }).map((_, index) => {
      ...
      return (
        <Star 
          key={index} 
          className={`w-4 h-4 ${
            filled || halfFilled ? "text-amber-400 fill-amber-400" : "text-gray-300"
          }`}
        />
      );
    });
  ...
  return (
    <div className="mb-6">
      ...
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg border border-purple-100/50 shadow-sm mb-6">
        <h3 className="text-sm font-semibold uppercase tracking-wide text-gray-700 mb-3">Product Features</h3>
        <ul className="space-y-2">
          ...
          <li className="flex items-center">
            <Check className="h-4 w-4 text-purple-600 mr-2" />
            <span className="text-sm">Cruelty-free and vegan-friendly</span>
          </li>
        </ul>
      </div>
    </div>
