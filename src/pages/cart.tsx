...
export default function Cart() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      router.push(`/success?session_id=${id}`);
    } catch (err: any) {
      setError(err.message);
    ...
    <div className="max-w-6xl mx-auto px-4 py-10">
      ...
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="bg-gradient-to-r from-purple-600 to-purple-800 py-6 px-8">
          <h1 className="text-3xl md:text-4xl font-bold text-white">Your Shopping Cart</h1>
          <p className="text-purple-200 mt-2">Review your items before checkout</p>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-16 px-4">
            ...
            <p className="text-gray-500 mb-8 max-w-md mx-auto">Discover our collection of beauty products and add your favorites to your cart.</p>
            <Link href="/products" className="inline-block bg-gradient-to-r from-purple-600 to-purple-700 text-white px-8 py-4 rounded-xl font-medium shadow-md hover:shadow-lg transform hover:-translate-y-0.5 transition-all duration-150">
              Explore Products
            </Link>
          </div>
        ) : (
          <div className="p-6 md:p-8">
            <div className="mb-10">
              ...
              {cartItems.map((item) => (
                <div key={item._id} className="bg-white border border-gray-100 rounded-xl mb-4 p-4 md:p-6 hover:shadow-lg transition-all duration-200 group relative">
                    ...
                    <div className="col-span-12 md:col-span-6 flex items-center space-x-4">
                      ...
                      <div>
                        <Link href={`/products/${item.productId}`}>
                          <h3 className="font-semibold text-gray-800 text-lg mb-1 group-hover:text-purple-700 transition-colors">{item.name}</h3>
                        </Link>
                        <div className="flex items-center space-x-2">
                          <span className="text-sm text-gray-500">#{item.productId.substring(0, 8)}</span>
                          <span className="inline-block w-2 h-2 rounded-full bg-purple-500"></span>
                          <span className="text-sm text-gray-500">{item.category_id || 'Cosmetics'}</span>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="flex items-center border border-gray-200 rounded-lg shadow-sm">
                        <button 
                          className={`w-8 h-8 flex items-center justify-center ${
                            updatingItem === item._id ? 'text-gray-400' : 'text-gray-600 hover:text-purple-700 hover:bg-purple-50'
                  ...
                  <button 
                    className={`absolute top-4 right-4 ${
                      removingItem === item._id ? 'text-gray-400' : 'text-gray-400 hover:text-red-500'
            ...
            <div className="grid md:grid-cols-5 gap-8 items-start">
              <div className="md:col-span-3">
                <div className="bg-purple-50 p-6 rounded-xl mb-6">
                  <h3 className="text-lg font-semibold text-gray-800 mb-3">Have a promo code?</h3>
                  <div className="flex">
                    <input 
                      type="text" 
                    ...
                    <button 
                      className={`px-4 py-2 bg-purple-600 text-white font-medium rounded-r-lg ${
                        applyingPromo ? 'opacity-70 cursor-wait' : 'hover:bg-purple-700'
                      } transition-colors`}
                ...
                <Link href="/products" className="inline-flex items-center text-purple-600 hover:text-purple-800 font-medium">
              ...
              <div className="md:col-span-2">
                <div className="bg-white border border-gray-100 rounded-xl shadow-lg p-6">
                  ...
                  <div className="border-t border-gray-200 pt-4 mb-6">
                    ...
                    <p className="text-sm text-gray-500 mt-1">Delivery expected in 3-5 business days</p>
                  </div>
                  ...
                  <button
                    ...
                    className={`w-full bg-gradient-to-r from-purple-600 to-purple-700 text-white py-4 px-6 rounded-xl font-medium flex items-center justify-center transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5 ${
                      processingCheckout || cartItems.length === 0 ? 'opacity-70 cursor-not-allowed' : ''
                    }`}
                  >
                    {processingCheckout ? (
                      <>
