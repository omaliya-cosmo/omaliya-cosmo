...
const AddToCartButton: React.FC<AddToCartButtonProps> = ({
  ...
  const handleAddToCart = async () => {
    ...
    // Add the product to the cart using the API with axios
    try {
  ...
  const handleBuyNow = async () => {
    ...
    // First add to cart using the cart API
    try {
      const response = await axios.post("/api/cart", {
        productId: product.id,
        quantity,
      ...
      if (!response.data) {
        throw new Error("Failed to add to cart for checkout");
      }

      console.log("Added to cart for immediate checkout:", response.data);
